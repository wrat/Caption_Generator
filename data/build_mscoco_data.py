...
ImageMetadata = namedtuple("ImageMetadata",
                           ["image_id", "object_feature","filename", "captions"])
...
def _to_sequence_example(image, decoder, vocab):
  ...
  context = tf.train.Features(feature={
      ...
      "image/object_feature": _float_feature_list(image.object_feature)
  })
...
def _process_dataset(name, images, vocab, num_shards):
  ...
  images = [ImageMetadata(image.image_id,image.object_feature, image.filename, [caption])
...
def load_image_object_features(object_feature_file):
  object_features = dict()

  with tf.gfile.FastGFile(object_feature_file,"r") as f:
    for each_line in f:
      image_id,feature_info = each_line.split[':']
      feature_info = feature_info.replace('\n','')
      feature_vector = [float(element) for element in feature_info.split(' ')]
      feature_vector = feature_vector[1:]
      object_features[int(image_id)] = feature_vector

  return object_features

def _load_and_process_metadata(captions_file, image_dir,object_feature_file):
  ...
  object_features = load_image_object_features(object_feature_file)
...
def main(unused_argv):
  ...
  mscoco_train_dataset = _load_and_process_metadata(FLAGS.train_captions_file,FLAGS.train_object_feature_file,
                                                    FLAGS.train_image_dir)
  mscoco_val_dataset = _load_and_process_metadata(FLAGS.val_captions_file,FLAGS.val_object_feature_file,
                                                  FLAGS.val_image_dir)
